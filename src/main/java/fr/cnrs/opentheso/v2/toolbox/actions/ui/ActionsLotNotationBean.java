...
import fr.cnrs.opentheso.v2.toolbox.actions.service.ActionsLotNotationService;
...
@Setter
@ViewScoped
@Named("v2ActionsLotNotationBean")
@RequiredArgsConstructor
public class ActionsLotNotationBean implements Serializable {

    private final ActionsLotNotationService notationService;
    private final ThesaurusContext thesaurusContext;
    private final UserSession userSession;
    private final ToolboxAccessPolicy toolboxAccessPolicy;

    private ActionsLotImportPanelState<ActionsLotNotationCandidate> importPanel = new ActionsLotImportPanelState<>();
    ...
    public void validateImport() {
        ...
        try {
            ActionsLotImportValidationResult<ActionsLotNotationCandidate> result = notationService.validate(
            ...
            if (!result.success()) {
                ...
                toast(result.validCount() + " notation(s) prête(s) à importer");
            }
        } finally {
    ...
    public void applyImport() {
        ...
        importPanel.setBusy(true);
        try {
            ActionsLotApplyResult result = notationService.applyImport(
    ...
    public void downloadImportTemplate() {
        writeDownload("modele-notations.csv", notationService.templateBytes());
    }
    ...
    private void updateImportPanel() {
        ...
        PrimeFaces.current().executeScript(
                "window.boSyncPanel && window.boSyncPanel('notations','import','" + safe + "')"
        );
    }
