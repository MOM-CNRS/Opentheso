...
class SessionLifecycleServiceTest {
    ...
    void homeUrl_handlesRootAndNonRootContext() {
        assertEquals("/v2/thesauri", service.homeUrl(null));
        assertEquals("/v2/thesauri", service.homeUrl("/"));
        assertEquals("/opentheso/v2/thesauri", service.homeUrl("/opentheso"));
        assertEquals("/opentheso/v2/thesauri", service.homeUrl("/opentheso/"));
        assertEquals("/opentheso/v2/thesauri?sessionExpired=1", service.homeUrlWithSessionExpired("/opentheso"));
    ...
    void expireAndRedirect_invalidatesSessionThenRedirects() throws Exception {
        ...
        verify(response).sendRedirect("/opentheso/v2/thesauri?sessionExpired=1");
    }
    ...
    void expireAndRedirect_worksWhenNoExistingSession() throws Exception {
        ...
        verify(response).sendRedirect("/v2/thesauri?sessionExpired=1");
    }
    ...
    void logoutAndRedirectFromFaces_invalidatesAndRedirectsWithLogoutFlag() throws Exception {
        ...
        verify(session).invalidate();
        verify(externalContext).redirect("/opentheso/v2/thesauri?logout=1");
    ...
    void clearAndRedirectFromFaces_invalidatesAndRedirectsHome() throws Exception {
        ...
        verify(session).invalidate();
        verify(externalContext).redirect("/opentheso/v2/thesauri");
        verify(facesContext).responseComplete();
    }
